...
author: nadia
date: 13-01-2026
description: "Can React Server Actions replace fetch for client-side data fetching? Let's investigate and find out."
image: /assets/server-actions-for-data-fetching/welcome.png
imageLarge: /assets/server-actions-for-data-fetching/welcome-large.png
title: 'Can You Fetch Data with React Server Actions?'
tags: react, performance
---

Can we use React Server Actions (now known as Server Functions) for data fetching? That's a very good question that [pops up](https://www.reddit.com/r/nextjs/comments/188dcfz/fetching_data_in_server_actions/) from [time](https://github.com/vercel/next.js/discussions/58674) to [time](https://www.robinwieruch.de/next-server-actions-fetch-data/). But is it really a good idea to _fetch_ data with Server Actions? "Actions" kinda imply that they are not for fetching. Why would anyone even consider it?
...
Here's a question for you. Considering that we can safely read the response, what exactly is stopping me from using that fancy wrapper to _fetch_ data? Can I do this?
...
Because we're discussing client-side fetching here, and [request memoization](https://nextjs.org/docs/app/guides/caching#request-memoization) is a __server__ feature. No one was going to memoize your `fetch` requests in `useEffect`, unless you use some nice [data fetching library](https://tanstack.com/query/latest/docs/framework/react/overview) that explicitly caches data on the client.
...
And finally, if you want to reproduce the numbers and you use Chrome, you need [Caddy](https://caddyserver.com/) to imitate HTTP/2 for parallel requests. It's because Chrome has a limit on [parallel connections with HTTP/1](https://medium.com/@hnasr/chromes-6-tcp-connections-limit-c199fe550af6), which is just 6.
...
<div className="scrollable-container">
  <table border="1" cellpadding="5" cellspacing="0">
    ...
    <tbody>
    <tr>
      <td><strong>LCP (title is visible)</strong></td>
      <td>500ms</td>
    </tr>
    <tr>
      <td><strong>All data is visible</strong></td>
      <td>1.7s</td>
    </tr>
    </tbody>
  </table>
...
Nope. And it's actually documented behaviour, if you read [React documentation](https://react.dev/reference/rsc/use-server) carefully. Very, _very_ carefully. Hidden inside `use server` documentation among all the other [caveats](https://react.dev/reference/rsc/use-server#caveats) you can find this: "Accordingly, frameworks implementing Server Functions typically process one action at a time..."
